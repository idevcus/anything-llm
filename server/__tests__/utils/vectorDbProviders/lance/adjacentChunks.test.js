 ...
 *
 * 이 테스트는 LanceDB의 getAdjacentChunks 메서드와
 * performSimilaritySearch의 adjacentChunks 파라미터를 테스트합니다.
 */

describe("LanceDb.adjacentChunks", () => {
  describe("getAdjacentChunks", () => {
    it("chunkIndex가 없는 경우 빈 결과를 반환해야 함", async () => {
      ...
      const result = await LanceDb.getAdjacentChunks({
        client: mockClient,
      ...
      expect(mockClient.openTable).not.toHaveBeenCalled();
    });
    ...
    it("chunkIndex=0일 때 음수 인덱스는 조회하지 않아야 함", async () => {
      ...
      await LanceDb.getAdjacentChunks({
        client: mockClient,
        namespace: "test-namespace",
        docId: "doc-123",
        chunkIndex: 0,
  ...
  describe("performSimilaritySearch - adjacentChunks integration", () => {
    it("adjacentChunks=0일 때 기존 검색 결과만 반환해야 함", async () => {
      ...
      LanceDb.namespaceExists = jest.fn().mockResolvedValue(true);
      ...
      // connect 메서드 mock
      LanceDb.connect = jest.fn().mockResolvedValue({ client: mockConnection });
      LanceDb.namespaceExists = jest.fn().mockResolvedValue(true);
      LanceDb.similarityResponse = jest.fn().mockResolvedValue(similarityResult);
      ...
      expect(LanceDb.similarityResponse).toHaveBeenCalledWith({
    ...
    it("adjacentChunks>0일 때 이웃 청크를 포함해야 함", async () => {
      ...
      LanceDb.getAdjacentChunks = jest.fn().mockResolvedValue(adjacentResult);
      ...
      expect(LanceDb.getAdjacentChunks).toHaveBeenCalledWith({
        client: mockConnection,
        namespace: "test-namespace",
        docId: "doc-123",
        chunkIndex: 2,
    ...
    it("chunkIndex가 없는 청크는 이웃 청크 조회를 스킵해야 함", async () => {
      ...
      LanceDb.connect = jest.fn().mockResolvedValue(mockConnection);
      LanceDb.namespaceExists = jest.fn().mockResolvedValue(true);
      LanceDb.similarityResponse = jest.fn().mockResolvedValue(similarityResult);

      LanceDb.getAdjacentChunks = jest.fn();
      ...
      expect(LanceDb.getAdjacentChunks).not.toHaveBeenCalled();
