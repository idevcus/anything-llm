 ...
 *
 * 이 테스트는 Zilliz의 getAdjacentChunks 메서드와
 * performSimilaritySearch의 adjacentChunks 파라미터를 테스트합니다.
 */

describe("Zilliz.adjacentChunks", () => {
  const { Zilliz } = require("../../../../utils/vectorDbProviders/zilliz");
  describe("getAdjacentChunks", () => {
    it("chunkIndex가 없는 경우 빈 결과를 반환해야 함", async () => {
      ...
      const result = await Zilliz.getAdjacentChunks({
        client: mockClient,
    ...
    it("adjacentCount=1일 때 앞뒤 1개씩의 청크를 조회해야 함", async () => {
      ...
      const queryParam = mockClient.query.mock.calls[0][0];
      expect(queryParam.collection_name).toBe("anythingllm_test_namespace");

      const expr = queryParam.expr;
      expect(expr).toContain('docId == "doc-123"');
      expect(expr).toContain("chunkIndex >= 0");
      expect(expr).toContain("chunkIndex <= 2");
      expect(expr).toContain("chunkIndex != 1");
    ...
    it("chunkIndex=0일 때 음수 인덱스는 조회하지 않아야 함", async () => {
      ...
      await Zilliz.getAdjacentChunks({
        client: mockClient,
        namespace: "test-namespace",
        docId: "doc-123",
        chunkIndex: 0,
  ...
  describe("performSimilaritySearch - adjacentChunks integration", () => {
    it("adjacentChunks=0일 때 기존 검색 결과만 반환해야 함", async () => {
      ...
      Zilliz.namespaceExists = jest.fn().mockResolvedValue(true);
      ...
      // connect 메서드 mock
      Zilliz.connect = jest.fn().mockResolvedValue({ client: mockClient });
      Zilliz.similarityResponse = jest.fn().mockResolvedValue(similarityResult);

      const result = await Zilliz.performSimilaritySearch({
        namespace: "test-namespace",
        input: "test query",
      ...
      expect(Zilliz.similarityResponse).toHaveBeenCalledWith({
        client: mockClient,
        namespace: "test-namespace",
        queryVector: [1, 2, 3],
        similarityThreshold: 0.25,
    ...
    it("adjacentChunks>0일 때 이웃 청크를 포함해야 함", async () => {
      ...
      Zilliz.getAdjacentChunks = jest.fn().mockResolvedValue(adjacentResult);
      ...
      expect(Zilliz.getAdjacentChunks).toHaveBeenCalledWith({
        client: mockClient,
        namespace: "test-namespace",
        docId: "doc-123",
        chunkIndex: 2,
    ...
    it("chunkIndex가 없는 청크는 이웃 청크 조회를 스킵해야 함", async () => {
      ...
      Zilliz.getAdjacentChunks = jest.fn();
      ...
      expect(Zilliz.getAdjacentChunks).not.toHaveBeenCalled();
